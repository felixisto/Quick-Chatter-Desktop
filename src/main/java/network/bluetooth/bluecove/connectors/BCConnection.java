 ...
 */
package network.bluetooth.bluecove.connectors;
...
public class BCConnection {
    ...
    public BCConnection(@NotNull BCAdapter adapter, @NotNull BEClient client, @NotNull UUID uuid, int retryCount) {
        _adapter = adapter;
        _client = client;
        _uuid = uuid;
    ...
    public BCConnection(@NotNull BCAdapter adapter, @NotNull BEClient client, @NotNull UUID uuid) {
    ...
    private void tryConnect(@NotNull Callback<BESocket> success, @NotNull Callback<Exception> failure) {
        ...
        if (_client.getDevice() instanceof BCClientDevice) {
            clientDevice = ((BCClientDevice)_client.getDevice());
        } else {
    ...
    private void startConnection(@NotNull String url, @NotNull Callback<BESocket> success, @NotNull Callback<Exception> failure) {
        try {
            ...
            success.perform(new BCSocket(streamConnection));
        } catch (Exception e) {
            failure.perform(e);
        }
    }
