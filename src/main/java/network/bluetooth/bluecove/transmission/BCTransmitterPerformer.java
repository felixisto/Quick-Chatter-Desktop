...
public class BCTransmitterPerformer implements LooperClient {
    private final @NotNull Object lock = new Object();

    public final @NotNull BCTransmissionLine line;
    private final @Nullable BCTransmitterPerformerDelegate _delegate;
    ...
    private final @NotNull BCTransmissionMessagePartBuilder _partsBuilder;

    private final @NotNull Timer _resetTimer = new Timer(TimeValue.buildSeconds(1));

    public static @NotNull BCTransmitterPerformer build(@NotNull BCTransmissionLine line, BCTransmitterPerformerDelegate delegate) {
        return new BCTransmitterPerformer(line, delegate);
    }

    BCTransmitterPerformer(@NotNull BCTransmissionLine line, @Nullable BCTransmitterPerformerDelegate delegate) {
        this.line = line;
        this._partsBuilder = new BCTransmissionMessagePartBuilder(line.getType());
    ...
    private @NotNull BuiltMessagesResult buildFullySentMessagesFromParts(@NotNull List<TransmissionMessagePart> parts) {
        ...
        for (TransmissionMessagePart part : parts) {
            if (part instanceof TransmissionMessagePart.Ping) {
                messages.add(new BCTransmissionMessage(part.getType()));
            } else {
                ...
                if (part instanceof TransmissionMessagePart.End) {
                    if (verifyPartsDataIntegrity(currentMessageParts)) {
                        BCTransmissionMessageBuilder builder = new BCTransmissionMessageBuilder(part.getType());
