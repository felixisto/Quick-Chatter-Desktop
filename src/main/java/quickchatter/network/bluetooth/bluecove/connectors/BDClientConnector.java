...
public class BDClientConnector implements BEConnector.Client {
    ...
    public BDClientConnector(@NotNull BEClient server, @NotNull BDAdapter adapter, int tryCount, @NotNull TimeValue retryDelay) {
        _server = server;
        _adapter = adapter;
    ...
    private void start(final @NotNull Callback<BESocket> success, final @NotNull Callback<Exception> failure) throws Exception, BEError {
        ...
        SimpleCallback completion = new SimpleCallback() {
            ...
            public void perform() {
                ...
                BDConnection connection = new BDConnection(_adapter, _server, getUUID());
